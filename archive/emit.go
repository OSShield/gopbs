...
// emitter walks a node tree and streams the v1 archive.
type emitter struct {
	w    *countWriter
	src  payloadSource
	warn func(Warning)
...
func (em *emitter) run(root *scan.Node) error {
	...
	return em.node(root, "", true)
}

func (em *emitter) node(n *scan.Node, apath string, isRoot bool) error {
	start := em.w.pos
	...
	if n.Kind == scan.KindHardlink {
		...
		if !ok {
			return fmt.Errorf("archive: hardlink %s: target %q not emitted before the link", apath, n.LinkTarget)
		}
		return em.w.write(pxar.AppendHardlink(em.scratch(), start-targetStart, n.LinkTarget))
	}
	...
	// payload header is the freshest observable value.
	var pl payload
	...
	case scan.KindDirectory:
		...
		for _, child := range n.Children {
			childStart := em.w.pos
			if err := em.node(child, joinArchive(apath, child.Name), false); err != nil {
				return err
			}
			items = append(items, pxar.GoodbyeItem{
				Hash:   pxar.Hash(child.Name),
				Start:  childStart,
				Length: em.w.pos - childStart,
			})
		}
		return em.w.write(pxar.AppendGoodbye(em.scratch(), items, entryStart, em.w.pos))
	...
	case scan.KindSymlink:
		return em.w.write(pxar.AppendSymlink(em.scratch(), n.LinkTarget))

	case scan.KindBlockDevice, scan.KindCharDevice:
		return em.w.write(pxar.AppendDevice(em.scratch(), n.Stat.Rdev))

	case scan.KindFifo, scan.KindSocket:
		return nil // the type bits in the entry's mode say it all
	}

	return fmt.Errorf("archive: unsupported node kind %d at %s", n.Kind, apath)
}
