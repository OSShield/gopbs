...
1. Wipes and regenerates the work directories `.test-source` (a randomized
   tree: nested dirs, mixed-size files), `.test-source-exclude` (the fixed
   exclusion fixture), `.test-pxar`, `.test-restore`.
2. `docker compose build`, then creates archives of the source tree with
   ...
   - `pbsexclude` / `gopbsexclude` / `pbssplitexclude` / `gopbssplitexclude` —
     v1 and split archives of the deterministic exclusion fixture
     `.test-source-exclude` (exclude_test.go), built with the same
     `--exclude` patterns and the fixture's `.pxarexclude` files.
...
| `TestExcludeComparison` | The exclusion fixture archived with patterns and `.pxarexclude` files restores to the same tree as the official CLI's, that tree is the expected one, and the recorded `.pxarexclude-cli` matches (absent from the split archive). |
| `TestExcludeBytes` | The exclusion archives are **byte-identical** to the official CLI's — v1 (synthetic `.pxarexclude-cli` entry, its position and metadata) and the split pair (JSON prelude). |
| `TestExcludeCatalog` | The exclusion archive's catalog omits excluded entries, lists `.pxarexclude-cli` with mtime 0, and matches the restored tree. |
...
| `pbs` / `pbssplit` / `pbsexclude` / `pbssplitexclude` | Official `pxar` CLI (from the `pbs-client` Debian repo) creating reference archives. |
| `tizbac` | Reference Go implementation creating `tizbac.pxar`. |
| `gopbs` / `gopbssplit` / `gopbsexclude` / `gopbssplitexclude` | `cmd/gopbs-pxar` built from the working tree. |
